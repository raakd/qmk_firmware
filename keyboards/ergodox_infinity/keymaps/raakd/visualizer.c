...
static void get_visualizer_layer_and_color(visualizer_state_t* state) {
    ...
    if (state->status.layer & 0x10) {
        // reddish?
        state->target_lcd_color = LCD_COLOR(20, saturation, brightness);
        state->layer_text = "GAMING (m)";
    }
        ...
        state->layer_text = "2 (TWO)";
    }
    else if (state->status.layer & 0x2) {
        // blue
        state->target_lcd_color = LCD_COLOR(168, saturation, brightness);
        state->layer_text = "1 (ONE)";
    }
    else {
        // white
        state->target_lcd_color = LCD_COLOR(0, 0, brightness);
        state->layer_text = "0 (ZERO)   ";
    }
}
